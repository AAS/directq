...
{
	if (!mod->cache.data)
		Sys_Error ("Mod_Extradata: no extra data (possibly called on a BSP model)");
...
void Mod_LoadTextures (lump_t *l)
{
	...
	for (i=0 ; i<m->nummiptex ; i++)
	{
		...
		{
			tx->d3d_Texture = D3D_LoadTexture (mt->name, tx->width, tx->height, (byte *)(mt+1), true, false);
		}
	}
...
void * Mod_LoadAliasFrame (void * pin, maliasframedesc_t *frame)
{
	...
	for (i=0 ; i<3 ; i++)
	{
		...
		aliasbboxmins[i] = frame->bboxmin.v[i] > aliasbboxmins[i] ? aliasbboxmins[i] : frame->bboxmin.v[i];
		aliasbboxmaxs[i] = frame->bboxmax.v[i] < aliasbboxmaxs[i] ? aliasbboxmaxs[i] : frame->bboxmax.v[i];
	}
...
void * Mod_LoadSpriteFrame (void * pin, mspriteframe_t **ppframe, int framenum)
{
	...
	*ppframe = pspriteframe;
	...
	pspriteframe->texture = D3D_LoadTexture (name, width, height, (byte *)(pinframe + 1), true, true);

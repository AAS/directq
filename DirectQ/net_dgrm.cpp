...
//		continue;
		...
		if (flags & NETFLAG_ACK)
		{
			...
			{
				memcpy(sock->sendMessage, sock->sendMessage+MAX_DATAGRAM2, sock->sendMessageLength);
				sock->sendNext = true;
			}
			else
			{
...
void NET_Stats_f (void)
{
	...
	{
		for (s = net_activeSockets; s; s = s->next)
			if (stricmp(Cmd_Argv(1), s->address) == 0)
				break;
		if (s == NULL)
			for (s = net_freeSockets; s; s = s->next)
				if (stricmp(Cmd_Argv(1), s->address) == 0)
...
//			return NULL;
	...
	strcpy(sock->address, net_DriverFunc.AddrToString(&clientaddr));
...
static void _Datagram_SearchForHosts (bool xmit)
{
	...
	while ((ret = net_DriverFunc.Read (net_DriverFunc.controlSock, net_message.data, net_message.maxsize, &readaddr)) > 0)
	{
		...
		strcpy(hostcache[n].cname, net_DriverFunc.AddrToString(&readaddr));

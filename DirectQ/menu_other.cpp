...
void Menu_LoadAvailableSkyboxes (void)
{
	...
	skybox_menulist = (char **) Pool_Game->Alloc ((listlen + 2) * sizeof (char *));
	...
	for (int i = 0; i < listlen; i++)
	{
		skybox_menulist[i + 1] = (char *) Pool_Game->Alloc (strlen (NewSkyboxList[i]) + 1);
	...
	skybox_menulist[0] = (char *) Pool_Game->Alloc (20);
...
void Menu_InitOptionsMenu (void)
{
	...
	menu_EffectsSimple.AddOption (new CQMenuCvarToggle ("Lock Alpha Sliders", &r_lockalpha, 0, 1));
	menu_EffectsSimple.AddOption (new CQMenuCvarSlider ("Water Alpha", &r_wateralpha, 0, 1, 0.1));
	menu_EffectsSimple.AddOption (TAG_WATERALPHA, new CQMenuCvarSlider ("Lava Alpha", &r_lavaalpha, 0, 1, 0.1));
	menu_EffectsSimple.AddOption (TAG_WATERALPHA, new CQMenuCvarSlider ("Slime Alpha", &r_slimealpha, 0, 1, 0.1));
	menu_EffectsSimple.AddOption (TAG_WATERALPHA, new CQMenuCvarSlider ("Teleport Alpha", &r_telealpha, 0, 1, 0.1));
...
void Menu_MapsCacheInfo (mapinfo_t *info, char *entlump)
{
	...
	while (1)
	{
		...
		{
			info->skybox = (char *) Pool_Game->Alloc (strlen (com_token) + 1);
			strcpy (info->skybox, com_token);
		}
		else if (!stricmp (key, "wad"))
		{
			info->wad = (char *) Pool_Game->Alloc (strlen (com_token) + 1);
...
bool ValidateMap (char *mapname, int itemnum)
{
	...
	// read it all in
	char *entlump = (char *) Pool_Temp->Alloc (bsphead.lumps[LUMP_ENTITIES].filelen);
	...
	for (int i = 0; i < (bsphead.lumps[LUMP_ENTITIES].filelen - 11); i++)
	{
		...
		{
			// map is valid
			menu_mapslist[itemnum].bspname = (char *) Pool_Game->Alloc (strlen (mapname) + 1);
...
void Menu_MapsPopulate (void)
{
	...
	int maplistlen = 0;
	menu_mapslist = (mapinfo_t *) Pool_Game->Alloc (listlen * sizeof (mapinfo_t));
	...
	spinbox_maps = (char **) Pool_Game->Alloc ((num_menumaps + 1) * sizeof (char *));
	spinbox_bsps = (char **) Pool_Game->Alloc ((num_menumaps + 1) * sizeof (char *));
...
void Menu_DemoPopulate (void)
{
	...
	demolist = (char **) Pool_Game->Alloc ((numdemos + 1) * sizeof (char *));
	...
	if (numdemos)
	{
		for (int i = 0; i < numdemos; i++)
		{
			...
			demolist[i] = (char *) Pool_Game->Alloc (thisdemolen + 1);
